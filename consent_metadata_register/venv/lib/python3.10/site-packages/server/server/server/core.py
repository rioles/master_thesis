...
import binascii
import os
from soc_settings.mclasses import ServerVerifier
from soc_settings.descr import CorrectPort
from soc_settings.config import *
from soc_settings.utils import send_message, get_message
from soc_settings.decorator import login_required
...
class MessageProcessor(threading.Thread, metaclass=ServerVerifier):
    ...
    def __init__(self, listen_address, listen_port, database):
        ...
        # Флаг продложения работы
        self.running = True

        # Словарь содержащий сопоставленные имена и соответсвующие им сокеты
        self.names = dict()
    ...
    # ищет клиента в словаре клиентов и удаляет его из списков и базы
    ...
    # Функция адресной отправки сообщения определенному клиенту. Принимает словарь сообщения, список зарегистрированных
    ...
    # Обработчик соощений от клиентов, принимает словарь - сообщение от клиента, проверяет корректность, отправляет
    ...
    def process_client_message(self, message, client):
        ...
        if ACTION in message and message[ACTION] == PRESENCE and TIME in message and USER in message:
            # Если это сообщение о присутсвии то вызываем функцию авторизации
            self.autorize_user(message, client)

        # Если это сообщение, от отправляем его получателю
        ...
        # Если это запрос публичного ключа пользователя
            ...
            # может быть, что ключа еще нет (пользователь никогда не логинился, тогда шлем 400)
            if response[DATA]:
                try:
                    send_message(client, response)
                except OSError:
    ...
    def autorize_user(self, message, sock):
        ...
        # Проверяем что пользователь зарегистрирован на сервере
            ...
            # Создаем хэш пароля и связки с рандомной строкой, сохряняем серверную версию ключа
