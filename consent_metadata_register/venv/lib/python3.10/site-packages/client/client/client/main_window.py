...
class ClientMainWindow(QMainWindow):
    ...
    def set_active_user(self):
        # Запрашиваем публичный ключ пользователя и создаем объект шифрования
        try:
    ...
    def message(self, message):
        ...
        # Декодируем строку, при ошибке выдаем сообщение и завершаем функцию
        try:
    ...
    def connection_lost(self):
        ...
        self.close()
    # Слот сообщения 205 - требовние соевером обновить справочники доступных пользователей и контактов
    # Может получить так, что удален текущий собеседник, надо проверить это и закрыть чат с предупреждением
    ...
    def sig_205(self):
        if self.current_chat and not self.database.check_user(self.current_chat):
            self.messages.warning(self, 'Сочувствую', 'К сожалению собеседник был удален с сервера')
            self.set_disabled_input()
            self.current_chat = None
        self.clients_list_update()
