...
# Класс - Транспорт, отвечае за взаимодействие с сервером
class ClientTransport(threading.Thread, QObject):
    # Сигналы: новое сообщение и потреря соединения
    ...
    def connection_init(self, port, ip):
        # Инициализация сокета и сообщения серверу о нашем появлении
        ...
        with socket_lock:
            ...
            # Посылаем серверу привественное сообщение и получаем ответ что все нормально
            try:
                ...
                if RESPONSE in ans:
                    if ans[RESPONSE] == 400:
                        ...
                        # Если все хорошо, то продолжаем процедуру авторизации
                        ans_data = ans[DATA]
    ...
    # Функция обрабатывающая сообщения от сервера. Ничего не возвращает. Генерирует исключени при ошибке
    ...
    # Функция обновляющая контакт-лист сервера
    ...
    # Функиця обновления таблицы известных пользователей
    ...
    # Фукнция сообщающая на сервер о добавлении нового контакта
    ...
    # Фукнция закрытия соединения. Отправляет сообщение о выходе
    ...
    def send_message(self, to, message):
        ...
        # Необходимо дожаться освобождения сокета для отправки сообщения
        with socket_lock:
    ...
    def run(self):
        ...
        while self.running:
            # Ждем секунду и снова пробуем захватить сокет
            # если сделать без задержки, то отправка может долго ждать освобождения сокета
            time.sleep(1)
            message = None
            with socket_lock:
                try:
