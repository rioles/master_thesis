...
def user_consent_datas():
    ...
    #schema_file = os.getenv('topic_name')
    bootstrap_server = os.getenv('bootstrap_server')
    all_data = {"consent":consent["consent_need"], "client": consent["client"], "consent_grant":data, "user_anip":anip_data}
    schema_dict = generate_schema_from_dict(all_data, "consent_data")
    schema_json_str = json.dumps(schema_dict)
    schema_type = "AVRO"
    client = SchemaClient(schema_url, subject_name, schema_json_str, schema_type)
    print(client)
    client.set_compatibility("FOWARD")
